...
execute if score $module_enabled.map_specific.all mla.main matches 1.. if score $module_enabled.map_specific.shrink mla.main matches 1.. if block ~ -63 ~ minecraft:white_stained_glass if score @s[tag=!no_shrink] mla.map_exclusive.shrink.loop matches ..45 run scoreboard players add @s mla.map_exclusive.shrink.loop 1
