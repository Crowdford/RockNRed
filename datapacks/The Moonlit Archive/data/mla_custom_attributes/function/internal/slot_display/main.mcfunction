...
item replace block 10000000 1 10000000 container.0 with clock[equippable={slot:"head",equip_sound:"block.candle.ambient"},tooltip_style="map_exclusive:common",tooltip_display={hidden_components:["enchantments"]},item_model="mla:survival/gui/mag",enchantments={"minecraft:binding_curse":1,"minecraft:vanishing_curse":1},max_stack_size=1,custom_data={mla_trinkets_data:{gui:1}}] 1
...
execute store result storage bbl:temp sudo_root.stats.current_hp int 1 run scoreboard players get @s mla.custom_attributes.hp.current
...
function mla_custom_attributes:internal/slot_display/head with storage bbl:temp sudo_root.stats
